...
- **描述**：
  ...
  - 实现六步审查：
    - _extract_core_loop（核心闭环提取）
    - _check_consistency（一致性检查）
    - _atomize_requirements（需求原子化）
    - _deduce_feasibility（技术可行性）
    - _detect_risks（风险检测）
    - _review_executability（可执行性）
...
## [ ] 任务 7：LangGraph 工作流开发
- **优先级**：P0
- **依赖**：任务 6
- **描述**：
  - 创建 workflows/review_workflow.py
  - 实现 initialize 节点（加载配置、设置默认值、创建目录、MCP 健康检查）
  - 实现 route_after_initialize（条件路由）
  - 实现 load_document（加载文档）
  - 实现 generate_spec（调用 Supervisor）
  - 实现 docreview（调用 DocReview）
  - 实现 evaluate_result（评估审查结果、停滞检测、历史压缩、成本检查）
  - 实现 revise_spec（调用 Supervisor 修订）
  - 实现 user_approval（中断点）
  - 实现 execute（调用 Supervisor 执行）
  - 实现 finalize（保存结果、清理资源）
  - 实现路由函数 route_after_evaluate、route_after_approval
  - 使用 SqliteSaver 持久化
...
- **描述**：
  ...
  - 实现通用选项：--doc-path、--task、--max-iterations、--output-dir、--spec-output、--verbose、--no-mcp、--model、--config
  - 实现 CLI 退出码处理（0/1/2/3/4）
  - 实现用户确认超时处理（asyncio.wait_for + aupdate_state）
  - 实现启动前预检查（.env、LLM_API_KEY）
- **验收标准覆盖**：AC1, AC6, AC7
- **测试需求**：
...
- **描述**：
  ...
  - 实现 LLM 定价表（支持 gpt-4o、gpt-4o-mini、gpt-4-turbo、claude-3.5-sonnet、claude-3-opus）
...
- **描述**：
  ...
  - 实现 Token/上下文窗口管理（摘要策略、分段审查）
  - 实现用户手动修订检测（spec_snapshot 对比）
  - 实现 SQLite checkpoint 完整性检查和损坏处理
- **验收标准覆盖**：AC14, AC15
- **测试需求**：
