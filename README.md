...
- Python 3.11+
- LLM API 密钥（OpenAI、Anthropic、阿里云通义千问等）
...
curl -X POST http://127.0.0.1:8000/ \
  ...
  -d '{"jsonrpc":"2.0","id":"1","method":"tools/call","params":{"name":"review_document","arguments":{"doc_path":"path/to/document.md"}}}'
```
...
本服务器遵循 [MCP 协议规范 v2024-11-05](https://modelcontextprotocol.io/specification/2024-11-05/)
...
```json
{
  ...
  "id": "2",
  "result": {
    "content": [{"type": "text", "text": "服务正常运行"}],
    "metadata": {
      "status": "healthy",
      "llm_available": true,
      "mcp_services": {"sequential_thinking": true, "context7": true}
    }
  }
}
```
...
│   │   └── stdio_server.py  # stdio 服务端（符合 MCP 协议规范）
│   ├── schemas/             # 数据模型
│   ├── state/               # 智能体状态管理
│   ├── utils/               # 工具函数
│   │   ├── logger.py
